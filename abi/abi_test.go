...
func (suite *TestAbiSuite) TestId() {
	...
	// Both formatted or un-formatted parts are equal
...
func (suite *TestAbiSuite) TestNew() {
	...
	newAbi, err := NewFromBytes(bytes)
	suite.Require().NoError(err)
	suite.Require().EqualValues(abi, *newAbi)
	...
	jsonAbi, err := NewFromInterface(json)
	suite.Require().NoError(err)
	suite.Require().EqualValues(abi, *jsonAbi)

	kv := key_value.Empty().
		Set("id", abi.Id).
		Set("bytes", data_type.AddJsonPrefix([]byte(abi.Body)))
	kvAbi, err := New(kv)
	suite.Require().NoError(err)
	suite.Require().EqualValues(abi, *kvAbi)

	// passing the value with a missing parameter should fail
	// in this case it's bytes.
	kv = key_value.Empty().
		Set("id", abi.Id)
	_, err = New(kv)
	suite.Require().Error(err)
